...
public class writeAction extends ActionSupport implements SessionAware {
	...
	public String execute() throws Exception {
		...
		for (int i = 0; i < uploads.size(); i++) {
			...
			System.out.println("업로드 사이즈 : " + uploads.size());
			...
			System.out.println("중간출력 : " + img_file);
			...
			String file_ext = getUploadsFileName().get(i).substring(getUploadsFileName().get(i).lastIndexOf('.') + 1,
					getUploadsFileName().get(i).length()); // .jpg만 짤라서 가져감 file_ext = .jpg
			...
			img_file = temp;
			System.out.println("나야나 나야나 : " + img_file);
